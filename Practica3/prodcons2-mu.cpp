...
// Implementación del problema del productor-consumidor con un proceso
// intermedio que gestiona un buffer finito y recibe peticiones
// en orden arbitrario
...
int producir(int num_orden){
   static int contador = num_orden * k;
   sleep_for(milliseconds(aleatorio<10,100>()));
   contador++;
   cout << "Productor " << num_orden << " ha producido valor " << contador
        << endl << flush;
   return contador;
}
...
void funcion_productor(int num_orden){
   for (unsigned int i = 0 ; i < k ; i++){
      ...
      // enviar valor
      cout << "Productor " << num_orden << " va a enviar valor " << valor_prod
           << endl << flush;
...
void funcion_consumidor(int num_orden){
   ...
   for (unsigned int i = 0 ; i < num_items / num_consumidores ; i++){
      ...
      cout << "Consumidor " << num_orden << " ha recibido valor " << valor_rec
           << endl << flush;
      consumir(valor_rec);
   }
}
...
void funcion_buffer(){
   int        buffer[tam_vector],      // buffer con celdas ocupadas y vacías
              ...
              etiq;                    // etiqueta del mensaje: 1 prod, 0 cons
...
int main(int argc, char * argv[]){
   ...
   if (num_procesos_esperado == num_procesos_actual){
      ...
      if (id_propio <= id_productor){
         num_orden = id_propio % num_productores;
         ...
         num_orden = id_propio % num_consumidores;
